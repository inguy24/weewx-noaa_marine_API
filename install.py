...
class MarineDataConfigurator:
    ...
    def _transform_fields_yaml_to_conf(self, selected_fields):
        ...
        # Process each module's selected fields
        modules = self.yaml_data.get('modules', {})
        for module_name, field_list in selected_fields.items():
            ...
            field_definitions = module_data.get('field_definitions', {})
            conf_mappings[module_name] = {}
            ...
            for field_name in field_list:
                try:
                    ...
                    database_table = self._determine_table_from_field(field_name, module_name)
                    ...
                    field_config = {
                        ...
                        'api_module': field_def.get('api_module', module_name),
                        'unit_group': field_def.get('unit_group', 'group_unknown'),
                        'unit_system': field_def.get('unit_system', 'METRIC')
                    }
                    
                    conf_mappings[module_name][field_name] = field_config
                    
                    # Update statistics
                    api_module = field_config.get('api_module', '').lower()
                    if 'coops' in api_module:
